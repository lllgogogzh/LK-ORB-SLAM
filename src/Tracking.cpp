...
bool debugLK = true;
    ...
    if(debugLK==true)
    {
        ...
        {
            //clock_t a = clock();
            //contrustion a Frame only plus id
            mCurrentFrame = Frame(true,timestamp);
            last_mnMatchesInliers = mnMatchesInliers;
            //before LK
            mpLK->setRefKeyFrame(mpLastKeyFrame);
            mpLK->setCurrentFrame(mCurrentFrame);
            //LK and PnP
            mpLK->useLKPnPRansac(imRGB,mK,mDistCoef,mnMatchesInliers);
            mpLK->PoseOptimizerLK();
            //mpFrameDrawer->LK = LKimg;
            mCurrentFrame.mTcw = mpLK->mTcw.clone();
            if(!mCurrentFrame.mTcw.empty())
            {
                cv::Mat tmpTvec=mCurrentFrame.mTcw.rowRange(0,3).clone();
                mpLK->mvTvec.push_back(tmpTvec);
            }
            //cv::Mat tmpTvec=mCurrentFrame.mTcw.rowRange(0,3).col(3).clone();
            //mpLK->mvTvec.push_back(tmpTvec);
            //clock_t b = clock();
            //cout<<"track LK optical flow use time:"<<1000*(float)(b-a)/CLOCKS_PER_SEC<<"ms"<<endl;
            mpFrameDrawer->Update(this);
            //mlFrameTimes.push_back(mCurrentFrame.mTimeStamp);
            mpLK->mvTimeStamp.push_back(mCurrentFrame.mTimeStamp);
...
//    const bool c2 = ((mnMatchesInliers<nRefMatches*thRefRatio || ratioMap<thMapRatio) && mnMatchesInliers>15);
    const bool c3 = mSensor!=System::MONOCULAR &&(mnMatchesInliers<600&& mnMatchesInliers>0);
    const bool c4 = mSensor!=System::MONOCULAR &&(mnMatchesInliers<200 && mnMatchesInliers>0);
